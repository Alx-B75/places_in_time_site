...
import { api } from '../api'
import FigureHeader from '../components/FigureHeader.jsx'

const LEGACY_AUTH_LOGIN_URL = 'https://places-in-time-history-chat-front.onrender.com/login'
const LEGACY_AUTH_REGISTER_URL = 'https://places-in-time-history-chat-front.onrender.com/register'
...
const GuestChat = () => {
  ...
  const querySlug = searchParams.get('figure_slug')
  const figureSlug = (routeSlug ?? querySlug ?? '').trim() || null
  ...
  }, [dispatch, figureSlug, placeSlug, sessionReady, activeFigureSlug, activePlaceSlug, mode])
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        ...
        console.error('[GUEST_CHAT] figure fetch failed', err)
        setFigure(null)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updatedRemaining = res?.remaining_questions ?? res?.remainingQuestions ?? normalizedRemaining ?? null
      ...
      if (errorType === 'guest_limit_reached') {
        ...
        const limitPayload = {
          ...
          maxQuestions: errDetail?.max_questions ?? errDetail?.maxQuestions ?? maxQuestions ?? null,
        }
        ...
        dispatch({
          type: 'SET_ERROR',
          payload: err?.message ?? 'Your guest pass is used up. Log in or register to keep chatting.',
        })
        ...
        dispatch({
          type: 'SET_ERROR',
          payload:
            err?.message ?? 'Our AI guide is temporarily unavailable because of high demand. Please try again later.',
        })
      } else if (errorType === 'network') {
        dispatch({
          type: 'SET_ERROR',
          payload: err?.message ?? 'We could not reach the chat service. Check your connection and try again.',
        })
  ...
  const disabled =
    sending || isPreparing || !sessionReady || guestLimitReached || quotaUnavailable || sessionMissing || !figureSlug

  const renderedMessages = messages.map((msg, index) => {
    const key = msg.id ?? `${msg.role}-${index}`
    const roleClass = msg.role === 'user' ? 'user' : 'assistant'
    return (
      <div key={key} className={`chat-message chat-message-${roleClass}`}>
  ...
  return (
    <section className="chat-page">
      ...
      <FigureHeader
        ...
        slug={figureSlug ?? ''}
      />
      {!figureSlug && <p className="chat-error">Choose a historical figure to begin chatting.</p>}
      ...
      {shouldShowLimitPanel && (
        <div className="chat-limit-panel">
          ...
          <div className="chat-limit-actions">
            <a className="button" href={LEGACY_AUTH_LOGIN_URL}>
              Log in
            </a>
            <a className="button" href={LEGACY_AUTH_REGISTER_URL}>
              Sign up
            </a>
          </div>
        </div>
      ...
      <div className="chat-shell">
        <div className="chat-messages">
          {messages.length === 0 && (
            <p className="chat-empty">Ask a question to begin — for example, "What was daily life like here?"</p>
          )}
          {renderedMessages}
